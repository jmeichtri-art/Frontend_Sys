...
import { Check, ChevronRight, ChevronsDownUp, ChevronsUpDown, Forklift, Settings, Package, ArrowRight, ArrowLeft, Search, X, Loader2, AlertCircle, BookMarked } from 'lucide-react';
...
export interface ConfiguratorWizardProps {
  /** Precarga la máquina, para reconfigurar una cotización existente. */
  initialMachineId?: number;
  /** Opciones ya elegidas que se preseleccionan una vez cargado el catálogo. */
  initialOptionIds?: number[];
  /** Texto del botón final. Por defecto "Generar Cotización". */
  submitLabel?: string;
  /** Al reconfigurar no tiene sentido guardar un template ni cambiar de compañía. */
  showTemplateActions?: boolean;
  showCompanySelector?: boolean;
  /** Qué hacer con la configuración terminada. */
...
export function ConfiguratorWizard({
  ...
  const handleGenerateQuotation = () => {
    // Al reconfigurar no se elige compañía: la cotización destino ya tiene la suya y el
    // consumidor ignora estos campos del draft.
    if (!selectedMachine) return;
    if (showCompanySelector && !selectedCompany) return;
  ...
  // Se guarda qué características están colapsadas, no cuáles están abiertas: así lo
  // que aparece nuevo (otro modelo, otra máquina) arranca expandido, que es el default.
  ...
  const expandAll   = (groups: CharacteristicGroup[]) =>
    setCollapsedChars((prev) => {
      const next = new Set(prev);
      for (const g of groups) next.delete(g.char.id);
      return next;
    });

  const collapseAll = (groups: CharacteristicGroup[]) =>
    setCollapsedChars((prev) => {
      const next = new Set(prev);
      for (const g of groups) next.add(g.char.id);
      return next;
    });
  ...
  // ── Precarga (reconfigurar una cotización existente) ──
  // La máquina se setea recién cuando llegó el listado; el efecto de [selectedMachine]
  // limpia las opciones, así que la preselección se aplica después, sobre machineOptions.
  useEffect(() => {
    if (!initialMachineId || !machines.length || selectedMachine) return;
    const found = machines.find((m) => Number(m.id) === Number(initialMachineId));
    if (found) setSelectedMachine(found);
  }, [initialMachineId, machines, selectedMachine]);

  const preselectionApplied = useRef(false);
  useEffect(() => {
    if (!machineOptions || !initialOptionIds?.length || preselectionApplied.current) return;
    const byId = new Map(machineOptions.options.map((o) => [Number(o.id), o]));
    const preselected: Record<number, Option> = {};
    for (const id of initialOptionIds) {
      const option = byId.get(Number(id));
      // Una opción que ya no existe o dejó de ser compatible simplemente no se preselecciona:
      // el asistente la va a pedir de nuevo como si fuera una configuración incompleta.
      if (option) preselected[Number(option.characteristicId)] = option;
    }
    setSelectedOptions(preselected);
    preselectionApplied.current = true;
  }, [machineOptions, initialOptionIds]);
  ...
  const renderGroups = (groups: CharacteristicGroup[]) =>
    groups.map(({ char, options }) => {
      const collapsed = collapsedChars.has(char.id);
      const chosen    = selectedOptions[char.id];
      return (
      <div key={char.id}>
        <button
          type="button"
        ...
        >
          ...
          />
          <h3 className="font-medium text-sm uppercase tracking-wider text-muted-foreground group-hover:text-foreground transition-colors">
            {char.name}
          </h3>
          {/* Colapsado igual se ve qué quedó elegido */}
          {chosen && (
            <span className={cn(
              'ml-auto text-xs truncate max-w-[45%] text-right',
              collapsed ? 'text-foreground font-medium' : 'text-muted-foreground',
            )}>
              {chosen.description}
            </span>
          )}
          {!chosen && collapsed && (
            <span className="ml-auto text-xs text-muted-foreground/60">Sin elegir</span>
          )}
        </button>
        <div className={cn('grid md:grid-cols-2 gap-3', collapsed && 'hidden')}>
          {options.map((opt) => {
        ...
        {step === 2 && (
          <div className="animate-fade-in space-y-6">
            ...
            {(() => {
              ...
              const allCollapsed = groups.every((g) => collapsedChars.has(g.char.id));
              return (
                <>
                  <div className="flex items-center justify-between -mb-2">
                    ...
                    <button
                      type="button"
                      onClick={() => (allCollapsed ? expandAll(groups) : collapseAll(groups))}
            ...
            <div className="space-y-4 max-w-2xl">
              ...
              <div className="flex flex-col gap-2">
                <Button
                  className="w-full"
                  size="lg"
                  disabled={showCompanySelector && !selectedCompany}
                ...
                {showTemplateActions && (templateSaved ? (
